...
~/x-ui.sh stop         # 停止 x-ui 面板
~/x-ui.sh restart      # 重启 x-ui 面板
~/x-ui.sh status       # 查看 x-ui 状态
~/x-ui.sh enable       # 设置 x-ui 开机自启
~/x-ui.sh disable      # 取消 x-ui 开机自启
~/x-ui.sh update       # 更新 x-ui 面板
~/x-ui.sh install      # 安装 x-ui 面板
~/x-ui.sh uninstall    # 卸载 x-ui 面板
~/x-ui.sh dokodemo     # 任意门中转菜单
```
